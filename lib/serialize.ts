...
export function workDTO(w: any): WorkDeclaration {
  return {
    ...
    yearComposed: w.yearComposed ?? "",
    soundCarrier: w.soundCarrier ?? "",
    financedByPublisher: w.financedByPublisher ?? "",
    publishingAgreementDate: w.publishingAgreementDate ?? "",
    ...
    fileNo: w.fileNo ?? "",
    factor: w.factor ?? "",
    registeredAt: w.registeredAt ? iso(w.registeredAt) : undefined,
    isrc: w.isrc,
    iswc: w.iswc,
