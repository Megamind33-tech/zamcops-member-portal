...
export default function SettingsScreen() {
  ...
  const { currentMember, logout, updateProfile, resetDemo } = useApp();
  ...
  const changePassword = (e: React.FormEvent) => {
    e.preventDefault();
    setPwMsg("");
    if (pw.current !== member.password) return setPwMsg("Current password is incorrect.");
  ...
  const doReset = () => {
    if (confirm("Reset all demo data to the seeded defaults? This will sign you out.")) {
      resetDemo();
      router.replace("/login");
    }
  };
  ...
  return (
    <div>
      ...
      <div className="space-y-5 px-4 py-4">
        ...
        <button
          onClick={doReset}
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white py-3 text-sm font-semibold text-slate-500"
        >
        ...
        <p className="pb-2 text-center text-[11px] text-slate-400">
          ZAMCOPS Member Portal · Prototype build
        </p>
      </div>
    </div>
  );
